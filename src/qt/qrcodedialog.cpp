...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("DOT")
    updateDisplayUnit();
}
...
{
    QString ret = QString("Dotcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DOT unit input in lnReqAmount, we generate the URI with DOT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DOT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
