...
// Copyright (c) 2014-2014 The Dotcoin developers
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uDOT
...
void ThreadScriptCheck() {
    RenameThread("kimdotcoin-scriptch");
...
unsigned char pchMessageStart[4] = { 0x16, 0x6f, 0x4f, 0x5d };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KimdotcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KimdotcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("KimdotcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kimdotcoin-miner");
    ...
    try { loop {
        ...
        printf("Running KimdotcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KimdotcoinMiner terminated\n");
        throw;
    }
}
