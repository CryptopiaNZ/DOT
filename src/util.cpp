...
#else
    const char* pszModule = "kimdotcoin";
#endif
    if (pex)
        return strprintf(
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No kimdotcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override kimdotcoin.conf
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Kimdotcoin will not work properly.");
